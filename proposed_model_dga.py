...
from gat_dga import gat_seq # use second version of gat
...
class TransformerProgramDecoder(torch.nn.Module):
    ...
    def make_instr_vectors(self, memory):
        """
        here!!
        """
        ##################################
        ...
        #print(instr_vectors_reshape.size())
        memory_repeat = memory.repeat_interleave(self.num_queries, dim=1) # [Len, RepeatBatch, Dim]
        
        return instr_vectors, instr_vectors_reshape, memory_repeat

    def forward(self, memory, tgt):
        
        instr_vectors, instr_vectors_reshape, memory_repeat = self.make_instr_vectors(memory)
        ...
        # print('before concat', instr_vectors_reshape.size(), tgt.size())
        tgt = torch.cat((instr_vectors_reshape, tgt), dim=0) # replace with our init values
        # print('after concat', tgt.size())
...
class ProposedModel(torch.nn.Module):
    def __init__(self):
        ...
        # print("text_vocab_embedding", self.text_vocab_embedding)
    ...
    def forward(self,
        ...
        # print('questions_encoded', questions_encoded.size())
        ...
        x_executed = self.gat_seq(x=x_encoded, edge_index=gt_scene_graphs.edge_index, edge_attr=edge_attr_encoded, instr_vectors=instr_vectors, batch=gt_scene_graphs.batch, questions=questions, sg=gt_scene_graphs)
...
if __name__ == "__main__":
    ...
    for data_batch in data_loader:
        ...
        # print("gt_scene_graphs.x", gt_scene_graphs.x.size(), gt_scene_graphs.x)
        ...
        # print("programs", programs.size(), programs)
        ##################################
