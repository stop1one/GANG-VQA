...
class TransformerPriorDecoder(torch.nn.Module):
    """
    Transformer Prior Decoder (new to GANG)
    Memory Input: question embedding
    Target Input: vertices (objects in SG) embedding
    """
    def __init__(self, ninp, nhead, nhid, nlayers, dropout=0.1):
        super(TransformerPriorDecoder, self).__init__()
        self.sg_emb_dim = 300
        self.embed_tgt = torch.nn.Linear(self.sg_emb_dim, ninp)
    ...
    def forward(self, memory, tgt):
        # tgt [x, 300] -> [x, num_q, ninp]
        num_q = memory.shape[1]
        embedded_tgt = self.embed_tgt(tgt) # [x, 300] -> [x, ninp]
        embedded_tgt = embedded_tgt.unsqueeze(1).repeat(1, num_q, 1)
        
        output = self.transformer_decoder(tgt=embedded_tgt, memory=memory) # [x, num_q, ninp]
        output = torch.norm(output, dim=0) # [num_q, ninp]
...
class ProposedModel(torch.nn.Module):
    def __init__(self):
        ...
        Prior Decoders
        """
        self.prior_decoder = TransformerPriorDecoder(
            ninp=self.question_hidden_dim,
            nhead=8, 
            nhid=4*self.question_hidden_dim, 
            nlayers=3, 
            dropout=0.1
        )
        self._lambda = 0.1 # weighted sum. the larger, the more applying priors
    ...
    def forward(self,
        ...
        """
        Guiding Attention Priors
        """
        # print("questions_encoded", questions_encoded.shape)
        # print("x_encoded", x_encoded.shape)
        prior_output = self.prior_decoder(memory=questions_encoded, tgt=x_encoded)
        # print("calcualte prior!", prior_output.shape)
        ...
        # print("graph_final_feature!", graph_final_feature.shape)
        # print(graph_final_feature)
        """
        Average Sum of GAT and Priors
        """
        graph_final_feature = self._lambda * prior_output + (1-self._lambda) * graph_final_feature
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        super(ProposedModel, self).load_state_dict(model_dict)
